...
<!-- Current Organization Display -->
    <div class="flex items-center gap-2 px-3 py-2 bg-base-200 rounded-lg">
        ...
        <span class="text-sm font-medium text-base-content/80">
            {{ user_current_organization_name() }} - {{ $user && $user->roles && $user->roles->first()?->name ? $user->roles->first()->name : 'No Role' }}
        </span>
    </div>
...
<div class="dropdown dropdown-end">
    <label tabindex="0" class="btn btn-info btn-sm gap-2">
        ...
        <span
            class="badge badge-xs">{{ $user && $user->roles && $user->roles->first()?->name ? $user->roles->first()->name : 'User' }}</span>
    </label>
    <ul tabindex="0"
